...
const form = useForm({
    ...
    priority: props.announcement.priority ?? "info",
    pinned: !!props.announcement.pinned,
    require_ack: !!props.announcement.require_ack,
    audience: props.announcement.audience ?? { roles: ["student"] },
    publish_at: props.announcement.publish_at ?? "",
    expires_at: props.announcement.expires_at ?? "",
});

const submit = () => {
    form.put(route("teacher.announcements.update", props.announcement.id));
};
</script>
